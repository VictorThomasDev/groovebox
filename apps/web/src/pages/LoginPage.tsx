...
export function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = mode === 'login'
        ...
        : await api.post<AuthResponse>('/api/auth/register', { email, password, username });
  ...
  return (
    <div style={{ maxWidth: 1440, margin: '0 auto', padding: '80px 40px', display: 'grid', gridTemplateColumns: '1fr 480px', gap: 80, alignItems: 'start' }}>
      {/* Left — editorial copy */}
      <div>
        <h1 style={{ fontFamily: 'Archivo Black, system-ui', fontSize: 'clamp(72px, 9vw, 124px)', lineHeight: 0.85, letterSpacing: '-0.035em', textTransform: 'uppercase', margin: 0 }}>
          {mode === 'login' ? <>Welcome<br /><em style={{ fontStyle: 'normal', color: 'var(--primary)' }}>back.</em></> : <>Start your<br /><em style={{ fontStyle: 'normal', color: 'var(--primary)' }}>collection.</em></>}
        </h1>
        ...
        <button
          ...
          style={{ marginTop: 32, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-soft)', textDecoration: 'underline' }}
        >
          {mode === 'login' ? 'No account? Register →' : 'Already have one? Log in →'}
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ borderTop: '3px solid var(--ink)', paddingTop: 32, display: 'flex', flexDirection: 'column', gap: 0 }}>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--ink-faint)', marginBottom: 24 }}>
          {mode === 'login' ? 'Log in to Groovebox' : 'Create account'}
        </div>
        ...
        <Field label="Password" type="password" value={password} onChange={setPassword} placeholder="••••••••" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} />
